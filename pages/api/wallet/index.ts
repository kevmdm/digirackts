      ...
      try {
        if (req.query) {
          ...
          switch (type) {
            ...
            default:
              res.status(400).json({ message: "bad query type not defined" });
              break;
          }
        }
      } catch (error) {
...
async function walletExistData(
...
async function addWalletEventData(
  req: NextApiRequest,
...
async function listWalletAssetApi(
  req: NextApiRequest,
  res: NextApiResponse<any>
) {
...
async function delistWalletAssetApi(
  req: NextApiRequest,
  res: NextApiResponse<any>
) {
...
async function relistWalletAssetApi(
  req: NextApiRequest,
  res: NextApiResponse<any>
) {
  const { wallet, listedAsset, event } = req.body;
